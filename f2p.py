...
class F2P:
    def __init__(self, _src_size, _dst_w, _dst_h, _scale, _interp_type):
    ...
    def generate_map(self,
        ...
        phi = np.arctan2(np.sqrt(Xx**2 + Xy**2), Xz)
        
        # 角度から入力画像の座標を計算
        r = (self.f  / self.scale) * phi
        self.map_u = (r * np.cos(theta) + self.Sh - 0.5).astype(np.float32)
        self.map_v = (r * np.sin(theta) + self.Sh - 0.5).astype(np.float32)
